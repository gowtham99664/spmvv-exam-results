...
const AdminDashboard = () => {
  ...
  const handleUpload = async (e) => {
    ...
    if (examHeldDate) {
      const selectedDate = new Date(examHeldDate + '-01'); // Add day for comparison
            ...
            <form onSubmit={handleUpload}>
              ...
              <div className="mb-4">
                ...
                <input
                  type="month"
                  value={examHeldDate}
                  onChange={(e) => setExamHeldDate(e.target.value)}
                  max={new Date().toISOString().slice(0, 7)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                  placeholder="YYYY-MM"
                />
                <p className="text-xs text-gray-500 mt-1">
                  Enter month and year for exam name generation (e.g., 2026-11 for November 2026). Must be a past or current date.
                </p>
              </div>
