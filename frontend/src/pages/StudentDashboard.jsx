...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Auto-refresh every 60 seconds for new results
                  ...
                  {notifications.map((notif) => (
                    <tr key={notif.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center text-sm">
                        {notif.result_id && (
                          <button
                            onClick={() => handleViewResults(notif.result_id, notif.id)}
          ...
          <div className="space-y-6">
            {Object.entries(groupedResults).sort().map(([key, group]) => (
              <div key={key} id={`result-${group.results[0]?.id}`} className="card">
                ...
                <h3 className="text-lg font-semibold text-gray-800 mb-3">Subject-wise Details</h3>
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Internal Marks</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">External Marks</th>
                      ...
                      {group.results.flatMap((result) => 
                        result.subjects.map((subject, idx) => (
                          <tr key={subject.id || idx}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{subject.subject_code}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{subject.internal_marks ?? 'N/A'}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{subject.external_marks ?? 'N/A'}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{subject.total_marks ?? 'N/A'}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-primary-600">{subject.grade}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{subject.credits ?? "N/A"}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
